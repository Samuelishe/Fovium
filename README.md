...
The current locally complete checkpoint is **`0.1.2.0001`**. R10-A-F1 keeps the bounded R10-A reference-sRGB foundation while making Dominant a representative substantial palette color rather than blindly the largest quantized bin, and makes the abstract Color Wash more expressive through a bounded `6×6` field and restrained perceptual tone mapping. Average remains mathematical, exact source identity still prevents stale styling, and automatic Matte uses the same representative Dominant authority. R9-A-F2 keeps Photo Presentation scale independent from Matte, R9-B-F1 keeps Slideshow independent from viewer layout mode, and R8-B-W1 remains enabled-by-default photograph-only Monitor Color Management for ordinary Windows SDR through the active display ICC and app-local Little CMS 2.19; hosted and non-Windows visual status must be evaluated separately.
