...
The current locally complete checkpoint is **`0.1.2.0002`**. R6-A-F1 presents Photo Info as a compact localized label/value card, adds a bounded set of standard photographic EXIF fields, fixes equivalent short-exposure rationals, and makes noninteractive surfaces of every floating overlay draggable while preserving control input and normalized placement. R10-A-F1 photo-derived styling, R9-A-F2 Matte-independent Photo Presentation scale, R9-B-F1 Slideshow layout independence, and R8-B-W1 ordinary-Windows-SDR Monitor Color Management remain unchanged; hosted and non-Windows visual status must be evaluated separately.
