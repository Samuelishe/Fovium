...
The current locally complete checkpoint is **`0.1.2.0003`**. R8-A-F1 adds Russian Color Picker display names through a separate stable-ID catalog with canonical-English fallback, and adds a localized Close control to the presenter dock through its existing visibility authority without clearing markup. Owner-accepted R6-A-F1 Photo Info/floating-drag behavior, R10-A-F1 photo-derived styling, R9-A-F2 Matte-independent Photo Presentation scale, R9-B-F1 Slideshow layout independence, and R8-B-W1 ordinary-Windows-SDR Monitor Color Management remain unchanged; hosted and non-Windows visual status must be evaluated separately.
