...
The current locally complete checkpoint is **`0.1.1.0000`**. R9-B adds Slideshow: `F5`, the checked context menu, and the live Presentation Settings control share one session authority; every slide receives its configured time from actual atomic publication, and Stop-at-end or Loop reuses the existing natural image sequence and Photo Presentation View. Historical R9-A at `0.1.0.0009` adds Photo Presentation View, while R9-A-F1 at `0.1.0.0010` adds its Settings discoverability and remembered, owner-centered Settings-window dimensions. R8-B-W1 remains enabled-by-default, photograph-only Monitor Color Management for ordinary Windows SDR through the active display ICC and app-local Little CMS 2.19; macOS, Linux, and Windows HDR monitor-aware output are not claimed.
