...
The current locally complete checkpoint is **`0.1.0.0010`**. Historical R9-A at `0.1.0.0009` adds Photo Presentation View: `F6` switches to an independently fitted, Matte-aware layout for every portrait, landscape, square, or panoramic photograph while preserving the ordinary viewer as the startup/default mode. R9-A-F1 adds live Settings activation plus remembered, validated, owner-centered Settings-window dimensions. R8-B-W1 remains enabled-by-default, photograph-only Monitor Color Management for ordinary Windows SDR through the active display ICC and app-local Little CMS 2.19; macOS, Linux, and Windows HDR monitor-aware output are not claimed.
