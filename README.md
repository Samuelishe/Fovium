...
The current locally complete checkpoint is **`0.1.1.0002`**. R9-A-F2 makes Photo Presentation scale depend only on the photograph, viewport, DPI, and Edge Margin; Matte remains independent decoration. R9-B-F1 keeps Slideshow independent from viewer layout mode and suppresses default focus chrome only on the focusable zero-UI viewport. R9-B at `0.1.1.0000` established publication-timed Stop-at-end/Loop navigation and one bounded prepared next frame. Historical R9-A at `0.1.0.0009` adds Photo Presentation View, while R9-A-F1 at `0.1.0.0010` adds its Settings discoverability and remembered, owner-centered Settings-window dimensions. R8-B-W1 remains enabled-by-default, photograph-only Monitor Color Management for ordinary Windows SDR through the active display ICC and app-local Little CMS 2.19; macOS, Linux, and Windows HDR monitor-aware output are not claimed.

This is an alpha milestone, not a feature-complete or stable release claim. Slideshow transitions, shuffle, music, and countdown UI; animated WebP/APNG; HEIF/AVIF sequences; high-bit-depth/HDR picker semantics; high-bit-depth/multipage TIFF; RAW; Advanced Metadata; persistent palettes; luminance/clipping histogram modes; metadata writing; full monitor-aware ICC; platform associations; markup export/object editing; and selected-reference A/B comparison are not implemented. The current matrix is [FORMAT-SUPPORT.md](docs/FORMAT-SUPPORT.md), Slideshow semantics are in [SLIDESHOW.md](docs/SLIDESHOW.md), and the canonical handoff is [PROJECT-STATE.md](docs/PROJECT-STATE.md).
