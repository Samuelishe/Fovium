...
Presenter primitives are stored in oriented image space and transformed through the resolved photo destination on every draw, so Fit, physical 100%, manual zoom, pan, resize, fullscreen, and Peek need no overlay-state rewrite. The pass clips to photograph bounds and uses direct Skia strokes after the photo without a viewport-sized offscreen surface. Blink selects the temporary image identity before taking its markup snapshot; missing comparison markup renders nothing rather than reusing current markup. The cursor highlight is a separate Avalonia viewport-space circle with physical radius converted by `RenderScaling`, so it can also appear over Stage.
