...
R9-A keeps the normal viewport model unchanged and adds one session-local alternative geometry policy. It first computes a physical inset equal to the configured percentage of the shorter physical viewport dimension, converts that inset to DIP through current `RenderScaling`, and then selects the largest physical scale at or below photographic 100% for which the canonical Matte outer bounds fit inside the inset rectangle. Matte disabled and Solid, Rounded, Soft, and Angular all reuse `StageGeometry` outer-extent semantics; Stage is not part of the fit. At impossible tiny geometry, inputs clamp to finite positive bounds and the viewport clips safely rather than producing negative/NaN rectangles.

The result is recomputed synchronously for current source, Matte changes, margin changes, resize, and fullscreen. It drives photo drawing, source-point sampling, and markup mapping together. It never creates an image, changes Color Management identity, or requests Little CMS work. F4 publication supplies the actually visible source and Stage snapshot, so portrait/landscape photo, Matte, Ambient, markup, and presented identity switch atomically. Leaving the mode restores ordinary semantic Fit.
