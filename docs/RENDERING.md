...
R8-B-W1 leaves viewport math authoritative. The same oriented destination rectangle, exact physical-100% alignment, nearest exact-pixel policy, and linear+mipmap fractional/downscale policy render into an unpremultiplied reference-sRGB bitmap. Little CMS converts those bytes once to monitor-device RGB; alpha is retained and output is premultiplied once into an untagged final image. The active identity owns a viewport/Fit-density full-source base plus a viewport detail with bounded overscan. The direct-Skia operation selects one coherent surface that contains the complete visible source region: exact or reusable detail first, otherwise the full-source base. It never presents a partial detail as the whole photograph or composites an unmanaged fill beneath it. During same-source/same-destination geometry refinement the selected device-RGB surface is temporarily reprojected through stored oriented-source coverage and current viewport geometry, remains spatially coherent with source-space markup and sampling, and is replaced by exact source/reference-to-destination output after interaction settles. It never labels device-RGB bytes as sRGB.
