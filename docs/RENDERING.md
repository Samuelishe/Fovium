...
R8-B-W1-F3 leaves viewport math and the existing direct-Skia draw path fully authoritative. Little CMS converts one encoded-size unpremultiplied reference-sRGB source to monitor-device RGB for each source/destination identity; alpha is retained, output is premultiplied once, and the final image is untagged. That managed image has the canonical encoded dimensions and orientation semantics. The draw operation changes only which same-size pixel source it supplies to the shared orientation matrix and exact-nearest versus fractional/downscale linear+mipmap branch. Fit, physical 100%, zoom, pan, resize, and Peek therefore cause no Color Management raster generation, resolution swap, coverage planning, debounce, or asynchronous sharpness replacement. Device-RGB pixels are never labeled sRGB and never become analysis input.

Source and destination changes remain strict by identity. For a same-destination source change, the selected target may advance while the previous complete rendered frame remains authoritative; photograph pixels, destination geometry, Matte/Ambient, markup selection, and `PresentedImageSource` switch only when the target managed source or recoverable fallback is renderable. The first source publishes no empty Matte placeholder. Geometry revisions retain the same source/destination and same-resolution managed source without proxy/exact quality states. The canonical source lease and current source-space mapping remain the authority for Color Picker, Histogram, Photo Info, markup, Peek, and Blink. Moving monitors therefore changes only photograph presentation pixels, not zoom, pan, physical 100%, source sampling, or overlay state.

## Photo Presentation layout

R9-A-F2 keeps the normal viewport model unchanged and corrects the session-local Photo Presentation geometry policy. It computes a physical inset equal to the configured percentage of the shorter physical viewport dimension, converts that inset to DIP through current `RenderScaling`, and selects the largest physical scale at or below photographic 100% for which the oriented photograph fits inside that inset rectangle. In physical pixels, available photo width/height are `viewport - 2 × margin`; Matte width, style, color, and enabled state are absent from this equation. At impossible tiny geometry, the effective margin clamps to retain finite positive photo bounds and the viewport clips safely rather than producing negative/NaN rectangles.

`PhotoPresentationLayout` returns only photograph destination, photograph presentation bounds, margin, and physical scale. Existing `StageGeometry` separately inflates the finished photo destination into Matte geometry; an extreme Matte may extend beyond the presentation margin and clips normally at the viewport instead of shrinking the photograph. Photo layout is recomputed synchronously for source, margin, resize, fullscreen, or DPI changes. Matte changes recompute only Stage decoration around the same photo bounds. The photo result drives drawing, source-point sampling, and markup mapping together; neither kind of geometry change creates an image, changes Color Management identity, or requests Little CMS work. F4 publication supplies the actually visible source and Stage snapshot, so portrait/landscape photo, Matte, Ambient, markup, and presented identity switch atomically. Leaving the mode restores ordinary semantic Fit.
