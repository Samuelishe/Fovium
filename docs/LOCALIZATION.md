...
The catalogs are embedded into the production assembly; no localization package is used. Context-menu commands, the file-picker title, controlled image-load errors, Settings/Viewing/Stage/Controls/About strings, Stage customization, command names, capture/conflict dialogs, and shortcut states resolve through catalog keys rather than scattered locale branches. Persisted command IDs are never localized.
