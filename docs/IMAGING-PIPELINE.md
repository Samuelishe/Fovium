...
R10-A adds a small reusable styling analysis at the successful canonical decode boundary. The existing off-UI decode work renders one oriented reference-sRGB thumbnail with at most a 96-pixel long edge, computes average/dominant/palette/spatial/boundary summaries, and attaches the immutable result to the exact `DecodedImage`. It is byte-accounted by the existing decoded cache and requires no file reopen, second full decode, monitor-managed presentation, or geometry-triggered work. Cancellation disposes an unpublished decoded candidate; a contained analysis failure leaves decode usable and activates truthful Stage fallback.
