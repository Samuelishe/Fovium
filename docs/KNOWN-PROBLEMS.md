...
| R1 automatic memory caps have only local smoke evidence | GC runtime allowance and conservative clamps are cross-platform APIs, but pressure behavior and native overhead vary by workload/platform | Measure representative large images and pressure scenarios before turning R1 constants into stable policy |
