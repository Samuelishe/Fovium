...
The derived raster contains the visible photograph intersection at current physical render scale plus bounded same-image source-space overscan. Canonical `DecodedImage`, retained encoded bytes, source color state, Color Picker, Histogram, Stage, Matte, Ambient, and markup are never destination-transformed. `AssumedSrgb`, `NormalizedSrgb`, and `NormalizedSrgbFromNclx` enter reference sRGB directly; `NormalizedNonSrgb` first uses its retained truthful Skia source color space; `EmbeddedProfileUnpreserved` retains the legacy approximate presentation with `UnsupportedSourceProfile`.

One worker owns at most one synchronous conversion plus one latest pending request. Generation and full source/destination/geometry identity reject stale completion. When only geometry changes, the previous same-source/same-destination managed raster is reprojected immediately as a presentation-only proxy from its stored oriented-source coverage; exact reference-sRGB-to-device processing replaces it after short coalesced refinement. Geometry-only refinement never intentionally hides the whole photograph, and proxy pixels are never input to Color Picker, Histogram, metadata, or other analysis. Initial, source-change, and destination-change transitions retain stricter identity behavior and never substitute unmanaged or wrong-monitor pixels. Missing engine, unavailable/invalid profile, disabled preference, unsupported source, non-Windows platform, and Windows Advanced Color/HDR are recoverable explicit states that preserve normal viewing.
