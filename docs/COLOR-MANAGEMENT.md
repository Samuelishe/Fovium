...
One worker owns at most one synchronous full-source conversion plus one latest pending source/destination request. Generation and source/destination identity reject stale completion; the key contains no viewport geometry. A new source or destination may prepare asynchronously, but an established source/destination performs zero CMM work for geometry-only interaction. The alternate managed image and canonical image enter the same orientation matrix and nearest-versus-linear/mipmap sampling branch, eliminating Black/partial/proxy/base frames, delayed sharpness replacement, and a second color-specific geometry model. Managed device pixels are never input to Color Picker, Histogram, metadata, or other analysis. Missing engine, unavailable/invalid profile, disabled preference, unsupported source, non-Windows platform, and Windows Advanced Color/HDR are recoverable explicit states that preserve normal viewing.
