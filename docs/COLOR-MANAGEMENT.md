...
R8-B-N1 is owner-accepted at `5155b7806703a657d89ab2923fd2936814a37a16`. Its reproducible owner under [`../eng/native/lcms2/`](../eng/native/lcms2/) pins Little CMS 2.19 and has green `win-x64`, `linux-x64`, and `osx-arm64` hosted build/audit/smoke evidence plus green normal CI.

## Windows ordinary-SDR destination management

R8-B-W1 uses that runtime only from `AppContext.BaseDirectory/runtimes/<rid>/native`, verifies encoded version `2190`, and never searches system Little CMS. On Windows with Advanced Color disabled, the real viewer HWND selects the monitor with the largest positive window intersection; an exact tie retains the current monitor and an off-screen/minimized placement retains the last valid monitor until placement becomes valid. Official Windows display APIs provide the assigned profile. The path is transient and is never persisted or logged; transform identity is SHA-256 of the admitted profile bytes plus output-mode state.

Destination admission is limited to 16 MiB and requires exact declared size, ICC signature, display-device class, RGB data, XYZ/Lab PCS, and a bounded tag table before Little CMS parses the profile. The production transform is reference sRGB BGRA8 unpremultiplied → monitor-device BGRA8 unpremultiplied, relative colorimetric, black-point compensation off, alpha copied; output is premultiplied exactly once and published as an untagged Skia image so no second transform occurs. VCGT is diagnostic-only and is never applied by Fovium.

The managed representation is one full-source, encoded-size device-RGB image for the active source/destination identity. It has the same dimensions and orientation semantics as the canonical decoded image, so the existing direct-Skia photo renderer remains the only Fit/100%/zoom/pan/resize/Peek geometry and sampling authority. Geometry changes neither regenerate Little CMS output nor replace a raster. Canonical `DecodedImage`, retained encoded bytes, source color state, Color Picker, Histogram, Stage, Matte, Ambient, and markup are never destination-transformed. `AssumedSrgb`, `NormalizedSrgb`, and `NormalizedSrgbFromNclx` read directly into full-source reference sRGB; `NormalizedNonSrgb` first uses its retained truthful Skia source color space during that read; `EmbeddedProfileUnpreserved` retains the legacy approximate presentation with `UnsupportedSourceProfile`.

One worker owns at most one synchronous full-source conversion plus one latest pending source/destination request. Generation and source/destination identity reject stale completion; the key contains no viewport geometry. A new source or destination may prepare asynchronously, but an established source/destination performs zero CMM work for geometry-only interaction. During a same-destination source transition, selected target and presented source are distinct: the previous complete photograph, geometry, Matte/Ambient, markup, and presented-image authority remain current until the next managed source is ready, then publish together. First load exposes neither photo nor Matte until the initial profile decision and corresponding presentation are ready. Recoverable managed failure publishes the target's legacy photograph and matching Matte as the same transaction. The alternate managed image and canonical image enter the same orientation matrix and nearest-versus-linear/mipmap sampling branch, eliminating Black/partial/proxy/base frames, delayed sharpness replacement, and a second color-specific geometry model. Managed device pixels are never input to Color Picker, Histogram, metadata, or other analysis. Missing engine, unavailable/invalid profile, disabled preference, unsupported source, non-Windows platform, and Windows Advanced Color/HDR are recoverable explicit states that preserve normal viewing.
...
R8-B-W1-F3 supersedes the earlier viewport/base/detail recommendation after owner video demonstrated Black, partial-coverage, and blur-to-sharp replacement regressions across W1/F1/F2. The selected representation is retained only for the current source/destination and is not inserted into the decoded-image LRU. Local optimized full-source measurements were about `327 ms` at 15 MP and `481 ms` at 24 MP; retained managed pixels add one BGRA surface (`60 MB` at 15 MP, `96 MB` at 24 MP), with one same-size temporary reference surface during preparation. The focused evidence owner is [`experiments/R8-B-W1-SEAMLESS-RENDERING-PROBE.md`](experiments/R8-B-W1-SEAMLESS-RENDERING-PROBE.md).
