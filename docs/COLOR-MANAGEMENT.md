...
The derived raster contains only the visible photograph intersection at current physical render scale. Canonical `DecodedImage`, retained encoded bytes, source color state, Color Picker, Histogram, Stage, Matte, Ambient, and markup are never destination-transformed. `AssumedSrgb`, `NormalizedSrgb`, and `NormalizedSrgbFromNclx` enter reference sRGB directly; `NormalizedNonSrgb` first uses its retained truthful Skia source color space; `EmbeddedProfileUnpreserved` retains the legacy approximate presentation with `UnsupportedSourceProfile`.

One worker owns at most one synchronous conversion plus one latest pending request. Generation and full source/destination/geometry identity reject stale completion. Until the matching raster is ready, the viewport shows only the identity-coherent Stage rather than flashing an unmanaged or wrong-monitor photograph. Missing engine, unavailable/invalid profile, disabled preference, unsupported source, non-Windows platform, and Windows Advanced Color/HDR are recoverable explicit states that preserve normal viewing.
