...
The managed presentation retains two bounded surfaces for the active source/destination identity: a full-source, viewport/Fit-density base capped at 32 MiB and a current viewport detail with `1.4×` overscan capped at 48 MiB. Overscan improves detail reuse but is not the coverage guarantee. Presentation selection uses a detail only when its stored oriented-source coverage contains the complete visible source region; otherwise it uses the full-source managed base for the whole photograph until exact detail is ready. Canonical `DecodedImage`, retained encoded bytes, source color state, Color Picker, Histogram, Stage, Matte, Ambient, and markup are never destination-transformed. `AssumedSrgb`, `NormalizedSrgb`, and `NormalizedSrgbFromNclx` enter reference sRGB directly; `NormalizedNonSrgb` first uses its retained truthful Skia source color space; `EmbeddedProfileUnpreserved` retains the legacy approximate presentation with `UnsupportedSourceProfile`.

One worker owns at most one synchronous conversion plus one latest pending request. Generation and full source/destination/geometry identity reject stale completion. When only geometry changes, a fully covering same-source/same-destination detail is reprojected immediately; if detail coverage is incomplete, the full-source managed base is reprojected instead. Coverage refinement starts immediately, while quality-only refinement retains the short coalescing debounce. Exact reference-sRGB-to-device processing replaces either transient surface after interaction settles. Geometry-only refinement never intentionally hides or incompletely covers the photograph, and base/detail pixels are never input to Color Picker, Histogram, metadata, or other analysis. Initial, source-change, and destination-change transitions retain stricter identity behavior and never substitute unmanaged or wrong-monitor pixels. Missing engine, unavailable/invalid profile, disabled preference, unsupported source, non-Windows platform, and Windows Advanced Color/HDR are recoverable explicit states that preserve normal viewing.
