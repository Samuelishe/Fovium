...
- Drawing-cursor and floating-overlay placement are pure presentation models. The viewport translates physical tool size through `RenderScaling`, suppresses the general Highlight while markup interaction is active, and reuses canonical pan for the non-history Hand tool. The Avalonia viewer owns only panel pointer capture/layout, while normalized client-relative placement persists through the existing schema-v2 settings owner. Project-owned vector icon geometry remains centralized secondary-UI data rather than a third-party dependency.
