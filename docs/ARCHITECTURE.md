...
R8-B-W1 adds a separate photograph-presentation boundary. `WindowsDisplayColorProfileProvider` resolves ordinary-SDR monitor state and bounded profile bytes without leaking paths; `LittleCmsRuntimeLocator` and the small direct API load only the Fovium-owned runtime; `SkiaLittleCmsPhotoRenderer` creates a reference-sRGB viewport-plus-overscan raster and one monitor-device raster; and `ManagedPhotoPresentationCoordinator` owns bounded latest-wins scheduling/publication. Each surface records oriented-source coverage, allowing the direct renderer to reproject a same-source/same-destination raster as a transient geometry proxy while exact refinement is pending; strict source and destination identity still reject reuse. `DecodedImage` and the presented-image lease remain canonical source authority for Color Picker, Histogram, Photo Info, Ambient, and Blink identity. Windows policy stays outside the cross-platform transform adapter, and non-Windows physical-monitor providers remain unimplemented.
