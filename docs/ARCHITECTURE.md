...
- The viewport exposes a retained, read-only presented-image lease and identity event without navigation authority. Photo Info consumes that boundary so canonical and Blink presentations share one identity-safe path; Peek does not publish a new identity. A project-owned metadata reader/model/cache/coordinator hides `MetadataExtractor`, parses retained encoded bytes in background work, and publishes only the latest visible presentation.
