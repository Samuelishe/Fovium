...
- Presenter state is a bounded, session-local model keyed by canonical image identity. Its immutable render snapshots contain oriented-source primitives and no Skia/Avalonia transforms; the viewport chooses canonical or Blink comparison identity and maps them in the existing photographic draw operation.
