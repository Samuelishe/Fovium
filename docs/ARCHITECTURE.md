...
R8-B-W1 adds a separate photograph-presentation boundary. `WindowsDisplayColorProfileProvider` resolves ordinary-SDR monitor state and bounded profile bytes without leaking paths; `LittleCmsRuntimeLocator` and the small direct API load only the Fovium-owned runtime; and `SkiaLittleCmsPhotoRenderer` uses the same reference-sRGB → monitor-device transform for a bounded full-source base and viewport detail. `ManagedPhotoPresentationCoordinator` owns at most those two surfaces, one active conversion, and one latest pending request. Each surface records oriented-source coverage; selection rejects incomplete detail coverage and reprojects the full-source base instead, while strict source and destination identity reject cross-image or cross-monitor reuse. `DecodedImage` and the presented-image lease remain canonical source authority for Color Picker, Histogram, Photo Info, Ambient, and Blink identity. Windows policy stays outside the cross-platform transform adapter, and non-Windows physical-monitor providers remain unimplemented.
