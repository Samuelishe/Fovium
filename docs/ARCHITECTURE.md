...
- Photo Presentation View owns only session mode state and one normalized edge-margin preference. The pure `PhotoPresentationLayout` receives current viewport/DPI, oriented source size, and canonical Stage/Matte settings, then returns photo and outer presentation bounds. `PhotoViewportControl` remains the single spatial authority and uses the same result for rendering, source-point mapping, markup scale, and tests; Color Management receives no presentation geometry.
...
R7-C adds one focused HEIF/AVIF backend and a deliberately small direct binding to the accepted libheif 1.23.1 API. A project-owned locator resolves only the exact Fovium runtime under `runtimes/<rid>/native` for proven `win-x64`, `linux-x64`, and `osx-arm64`; it never searches system paths. The backend identifies HEVC/AV1 primary content natively, validates still-image count, bit depth, HDR signaling, dimensions, and memory admission before pixel decode, then copies the temporary native RGBA plane directly into the common BGRA8888/Premul bitmap and releases every native owner. A missing or invalid bundle disables only this backend. No libheif type, handle, error enum, or runtime path escapes imaging.

R8-A keeps color inspection project-owned and behind existing presentation boundaries. `PhotoViewportControl` captures one retained currently presented image plus the exact oriented source pixel; `PhotoColorSampler` reads one BGRA8888/Premul pixel and converts only that sample to reference sRGB when needed; `ColorPickerSession` owns visibility/current sample/bounded history; and the floating overlay consumes immutable `ColorSample` values. The embedded catalog and small OKLab matcher have no service, database, general color framework, or new production package. Monitor destination transforms remain outside this subsystem.

R8-B-W1 adds a separate photograph-color boundary. `WindowsDisplayColorProfileProvider` resolves ordinary-SDR monitor state and bounded profile bytes without leaking paths; `LittleCmsRuntimeLocator` and the small direct API load only the Fovium-owned runtime; and `SkiaLittleCmsPhotoRenderer` creates one encoded-size device-RGB source for the active source/destination identity. `ManagedPhotoPresentationCoordinator` owns one current source, one active conversion, and one latest pending source/destination request. `SkiaPhotoDrawOperation` then draws either the canonical or managed same-size source through exactly the same orientation, Fit/100%, pan/zoom, and sampling code. No viewport geometry enters the Color Management key. `DecodedImage` and the presented-image lease remain canonical source authority for Color Picker, Histogram, Photo Info, Ambient, and Blink identity. Windows policy stays outside the cross-platform transform adapter, and non-Windows physical-monitor providers remain unimplemented.

R9-A adds no renderer or image representation. `PhotoPresentationLayout` solves one synchronous Matte-inclusive Fit from the actually presented source and current viewport, while the existing direct-Skia operation remains the only photo renderer. Activation state belongs to the viewer session; only edge-margin configuration is serialized. New-source layout enters the same F4 atomic publication as pixels, Matte, overlays, and `PresentedImageSource`, so selected target state cannot leak into a partially published frame. Future Slideshow may drive navigation but must reuse this layout mode rather than duplicate it.
