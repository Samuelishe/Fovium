...
R9-B Slideshow is the locally complete current product checkpoint at `0.1.1.0000`, following owner-accepted R9-A-F1 `0.1.0.0010` and historical R9-A `0.1.0.0009`. The session-local `viewer.toggleSlideshow` (`F5` by default), checked context-menu entry, and live Presentation Settings control share one timer/navigation authority. Slideshow starts from the actually presented image, drives the existing natural/failure-skipping sequence, and automatically enables the existing Photo Presentation View without creating another layout path. Duration (`5 s` default, `1–60 s`) and Stop-at-end/Loop configuration persist; running state, index, deadline, prepared identity, and temporary Photo Presentation ownership do not. Every countdown begins at F4 atomic publication of the fully ready frame. One optional exact source/destination managed next is bounded to `128 MiB` and current plus next to `256 MiB`; rejection affects latency rather than correctness or sequence membership.
...
R9-B is locally complete and awaits owner review/commit plus normal hosted CI. Deterministic tests cover publication-based timing, slow preparation without backlog, stale/manual races, bounded failure/wrap traversal, Stop-at-end, quiescent one-viable Loop, destination identity, atomic prepared consumption, memory admission, persistence, localization, commands, and Escape precedence. Windows Release smoke on a mixed-orientation sequence proved Stop-at-end, two Loop wraps, manual Right reset, F5/Esc stop, F11 continuity, checked context-menu and live Russian Settings state, `geometryRequests=0`, and `matteWithoutPhotoFrames=0`. Real CMM preparation measured `25.65 ms` for 0.96 MP, `291.40 ms` for 15 MP, and `463.64 ms` for 24 MP; prepared-hit timer-expiry-to-visible waits were approximately `0.24–3.37 ms`, with no redundant conversions in the verified Loop run. The canonical contract and remaining bounds are in [`SLIDESHOW.md`](SLIDESHOW.md).
