...
R10-A Photo-derived styling foundation is the locally complete current product checkpoint at `0.1.2.0000`, following R9-A-F2 `0.1.1.0002`. Every decoded photograph receives one bounded deterministic reference-sRGB analysis off the UI thread. Average, Dominant, and abstract Color Wash backgrounds, presentation-safe Average/Dominant Matte color, and one-physical-pixel Hairline Auto separation consume only the exact rendered source's immutable result. Geometry, Slideshow scheduling, Color Picker, Histogram, and Color Management domains remain independent.
...
R10-A is locally complete and awaits owner review/commit plus hosted verification after push. Pure and viewer tests cover average/dominant/palette/spatial results, deterministic normalization, byte-accounted decoded ownership, decode cancellation, identity mismatch fallback, navigation/Blink/Peek/Slideshow policy, persisted settings, Stage viewport invariance, and source-domain Picker/Histogram regressions. Windows Release evidence covers a public-domain real-photo corpus plus project-generated 15/24 MP inputs; the final five-image run measured the 96-pixel analysis at 3.40–17.49 ms with 336 retained managed bytes. Its one cached 64×64 wash adds 16,384 native bytes, for 16,720 styling bytes per decoded image. Rendered visual artifacts confirm distinct Average/Dominant fills, smooth abstract Color Wash, restrained auto Matte, and Hairline separation. Hosted status is not claimed before owner push.
