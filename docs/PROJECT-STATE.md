...
R6-A-F1 Photo Info and shared floating-overlay UX correction is the locally complete current product checkpoint at `0.1.2.0002`, following R10-A-F1 `0.1.2.0001`. Photo Info now presents sparse localized label/value rows, maps a bounded set of additional standard photographic EXIF fields, and formats equivalent short-exposure rationals consistently. Photo Info, Histogram, Color Picker, and the markup dock can be dragged from any noninteractive visible surface while their buttons and sliders retain ordinary input behavior. The accepted R10-A-F1 photo-derived styling behavior remains unchanged.
...
R10-A-F1 and its deterministic Photo Info concurrency-test correction are committed, pushed, and hosted-green on Windows, Ubuntu, and macOS; the separate native libheif runtime workflow is also green. R6-A-F1 is locally complete and awaits owner review, commit, push, and a new hosted rerun, so hosted-green status is not yet claimed for `0.1.2.0002`. Focused evidence covers equivalent reduced and unreduced shutter rationals, extended project-owned EXIF mapping, localized Photo Info rows/tooltips, noninteractive drag origins, interactive-control exclusions, and the unchanged normalized placement/clamping geometry.
