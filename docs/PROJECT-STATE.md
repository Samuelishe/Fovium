...
R8-B-W1 is committed at `144c08ca83080961e85bba06eb64505e6f39b6ba` and its first interaction correction is committed at `011a8f143299dd356937b66fca5073ccb67c59f1`; R8-B-W1-F2 is locally ready for owner verification at `0.1.0.0008`. On ordinary Windows SDR output, enabled-by-default photograph-only Monitor Color Management resolves the active display through the real viewer HWND, admits the assigned bounded RGB display ICC, and creates a reference-sRGB → monitor-device RGB base/detail presentation through app-local Little CMS 2.19. The full-source managed base prevents incomplete photograph coverage when viewport detail is outrun by zoom-out, pan, resize, or Peek. Advanced Color/HDR, invalid/unavailable profiles, unsupported source meaning, and missing native runtime retain the existing presentation recoverably. Color Picker remains reference sRGB and Histogram remains source-domain. R8-B-P1 is accepted at `932c1285394d6bfa6c10cc75c1810c882eef7c6c`; the test correction is accepted at `90d733254790d152cc5d298f39c23e2b8171b249`; R8-B-N1 is accepted at `5155b7806703a657d89ab2923fd2936814a37a16` with green normal and native hosted matrices.
...
R8-B-W1-F2 is the current owner-review focus. Local Windows evidence covers real HWND/profile resolution, Advanced Color disabled classification, app-local runtime loading, production matrix/TRC and LUT destination transforms, alpha, bounded latest-wins/coalescing, missing-runtime fallback, six-format managed navigation, deterministic full-coverage zoom-out races, and a 15 MP 1920×1080 zoom/pan/Fit/100%/Peek/resize soak with zero managed-incomplete and geometry-black frames. Acceptance still requires owner review/push followed by normal Windows/Linux/macOS CI and the extended native Little CMS `win-x64`/`linux-x64`/`osx-arm64` product-interop matrix. macOS, Linux/X11/Wayland, Windows Advanced Color/HDR, high-depth output, and real multi-monitor hardware transition evidence remain separate work.
...
- Windows ordinary-SDR photograph presentation through the assigned active-monitor RGB ICC and app-local Little CMS 2.19, with one bounded full-source viewport-density base plus one overscanned viewport detail, complete-coverage same-source/same-destination geometry proxies, exact-refinement latest-wins/coalescing, explicit fallback state, and no monitor transform of Stage, Matte, Ambient, markup, Color Picker, or Histogram.
