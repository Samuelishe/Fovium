...
R9-B-F1 Slideshow correction is the locally complete current product checkpoint at `0.1.1.0001`, following R9-B `0.1.1.0000`. The session-local `viewer.toggleSlideshow` (`F5` by default), checked context-menu entry, and live Presentation Settings control share one timer/navigation authority, but Slideshow never owns or changes the viewer layout mode. Normal Keep-current-scale/Fit-each-image navigation and independently toggled Photo Presentation remain authoritative in all four running/mode combinations. Duration (`5 s` default, `1–60 s`) and Stop-at-end/Loop configuration persist; running state, index, deadline, and prepared identity do not. Every countdown still begins at F4 atomic publication of the fully ready frame. One optional exact source/destination managed next remains bounded to `128 MiB` and current plus next to `256 MiB`; rejection affects latency rather than correctness or sequence membership. The focusable main viewport now explicitly suppresses its local Avalonia focus adorner while retaining logical keyboard focus.
...
R9-B-F1 is locally complete and awaits owner review/commit plus hosted verification after push. Deterministic tests retain the accepted timing/preload/end/manual-navigation coverage and add slideshow/view-mode independence plus a focusable-without-focus-adorners viewport invariant. Windows Release stress covers repeated F5/manual Left/Right plus F6, F11, context-menu, Settings, and focus loss/reacquisition without the prior inner focus rectangle; `geometryRequests=0` and `matteWithoutPhotoFrames=0` remain required. The accepted R9-B preparation evidence remains `25.65 ms` for 0.96 MP, `291.40 ms` for 15 MP, and `463.64 ms` for 24 MP, with prepared-hit waits of approximately `0.24–3.37 ms` and no redundant conversions in the verified Loop run. The canonical contract and remaining bounds are in [`SLIDESHOW.md`](SLIDESHOW.md).
