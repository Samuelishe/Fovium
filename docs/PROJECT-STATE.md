...
R9-A-F1 is locally accepted and its version-policy correction is the current bookkeeping focus. Pure coverage includes seven required aspect ratios, every Matte style, RenderScaling `1.00`/`1.25`/`1.50`/`2.00`, command/input suppression, session persistence boundaries, independent navigation layout, live Matte/margin/resize/fullscreen recomputation, F4 atomic portrait/landscape publication, live Settings/F6 state synchronization, and bounded logical Settings-window size restoration. A local Windows Release smoke used the app-local Little CMS runtime and bright Matte across windowed/fullscreen landscape and 15/24 MP portrait transitions: `geometryRequests=0`, `matteWithoutPhotoFrames=0`, five managed conversions for source preparation only. The R9-A-F1 Russian Settings smoke also proved `920×680 DIP` first-open size, live F6/checkbox synchronization, restored user dimensions, owner centering, and session reset to Off after restart. Blink is deliberately unavailable while Photo Presentation View is active rather than showing comparison geometry that has not been independently integrated. Owner commit/push and normal Windows/Linux/macOS hosted CI remain required; R9-B Slideshow is the selected next standalone feature, planned for `0.1.1.0000`, and is not implemented.
