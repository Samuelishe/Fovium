...
R7-C and its R7-C-N1 prerequisite are owner-accepted. The accepted prerequisite commit is `c4dba80bd23534f372ae09f9285c0e1c5991d5e3`, and normal hosted CI plus native/product integration are green across Windows/Linux/macOS and `win-x64`/`linux-x64`/`osx-arm64`. R8-A and the bounded R8-B-P1 architecture probe are also owner-accepted. R8-B-N1 is locally ready for hosted verification: the separate source-pinned Little CMS 2.19 owner performs an app-local clean Windows build, dependency/architecture audit, build-prefix-unavailable load, matrix/TRC and actual CLUT transforms, malformed-profile checks, and 100 repeated smokes. Required `linux-x64` and `osx-arm64` results remain pending the dedicated hosted matrix. Production transforms remain disabled and version stays `0.1.0.0007`; only after hosted N1 acceptance may the separately scoped Windows SDR R8-B-W1 stage begin.
...
- R8-B-P1 proves Skia 3.119.4 matrix/TRC conversion but also proves that it rejects a valid ICC v4 LUT display profile accepted by Little CMS 2.19. R8-B-N1 now has local Windows supply-chain evidence, but required Linux/macOS hosted native evidence is pending and no engine is integrated into production. macOS compositor evidence and Linux X11/Wayland platform paths also remain unvalidated.
