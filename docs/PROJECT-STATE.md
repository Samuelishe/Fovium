...
R10-A-F1 Representative Dominant and Color Wash visual tuning is the locally complete current product checkpoint at `0.1.2.0001`, following R10-A `0.1.2.0000`. Every decoded photograph still receives one bounded deterministic reference-sRGB analysis off the UI thread. Average remains mathematical; Dominant now selects a substantial representative palette family using population plus bounded perceptual chroma/lightness weighting; Color Wash uses a modestly richer `6×6` spatial field and bounded OKLab presentation mapping. Automatic Dominant Matte shares the same selector. Geometry, Slideshow scheduling, Color Picker, Histogram, and Color Management domains remain independent.
...
R10-A-F1 product work is committed and pushed. Its post-push hosted CI passed Windows and Ubuntu; the macOS build passed, while the test step exposed one scheduler-sensitive Photo Info test wait rather than a product-semantics failure. The local test-only correction now uses controlled inline completion to prove both stale metadata results were processed, and awaits owner review/commit/push plus a hosted rerun; full hosted-green status is not yet claimed. Synthetic tests distinguish substantial chromatic regions from tiny accents and prove neutral/dark/high-key behavior, raw-palette preservation, stable ordering, shared Dominant Matte authority, and bounded wash tuning; existing viewer tests retain identity, navigation/Blink/Peek/Slideshow, geometry reuse, and Picker/Histogram domain coverage. Windows Release evidence covers ten real/generated visual categories: red-car gray/black failures are replaced by representative red/burgundy/olive tones, sky remains blue, beige and grayscale remain plausible neutrals, night remains dark, and a 5% red accent does not capture the Stage. `4×4/6×6/8×8` comparison selected `6×6`; the final five-image run measured `8.37–20.03 ms`, 396 managed bytes, and 16,780 total styling bytes with the unchanged 16,384-byte wash.
