...
R9-A-F2 Photo Presentation geometry correction is the locally complete current product checkpoint at `0.1.1.0002`, following R9-B-F1 `0.1.1.0001`. Photo Presentation fits only the oriented photograph inside its persisted physical Edge Margin; Matte is derived afterward around the finished photo destination and cannot change its scale, source mapping, or markup alignment. Width, enabled state, Solid/Rounded/Soft/Angular style, and color remain ordinary Stage settings shared with Normal Viewer. Extreme Matte may extend beyond the presentation margin and clips at the viewport. Slideshow remains independent timed navigation over whichever viewer mode is active, and Color Management remains geometry-independent.
...
R9-A-F2 is locally complete and awaits owner review/commit plus hosted verification after push. Deterministic layout tests cover Matte toggle/width/style/color invariance, the portrait `32→64→100→147→32` regression, separate edge-margin resizing, mixed orientations, viewport changes, source mapping, and existing Stage outer geometry. Windows Release smoke repeats the OWNER scenario in Normal Viewer and Photo Presentation, including fullscreen; only Edge Margin changes photo size. Atomic publication retains `matteWithoutPhotoFrames=0`, and geometry-only Color Management remains `geometryRequests=0`. Slideshow scheduling, prepared-next memory, formats, and Color Management architecture are unchanged.
