...
R8-B-W1-F4 is locally ready for owner verification at unchanged `0.1.0.0008`, on F3 baseline `eb654360339a2fde3369acaca1f86319243dd17a`. W1/F1/F2 respectively exposed Black, incomplete-coverage, and blur-to-sharp raster-replacement regressions because monitor pixels were keyed by viewport geometry; accepted F3 replaced that family with one encoded-size managed source keyed only by source and destination. F4 makes new-source publication coherent: the previous photograph, geometry, Matte/Ambient, markup, and presented-image authority remain current while the next full-source managed image prepares, then switch together. First load waits for the initial display-profile decision without exposing an empty Matte. Advanced Color/HDR, invalid/unavailable profiles, unsupported source meaning, and missing native runtime retain the existing presentation recoverably. Color Picker remains reference sRGB and Histogram remains source-domain. R8-B-P1 is accepted at `932c1285394d6bfa6c10cc75c1810c882eef7c6c`; the test correction is accepted at `90d733254790d152cc5d298f39c23e2b8171b249`; R8-B-N1 is accepted at `5155b7806703a657d89ab2923fd2936814a37a16` with green normal and native hosted matrices.
...
R8-B-W1-F4 is the current owner-review focus. Local Windows evidence covers real HWND/profile resolution, app-local runtime loading, deterministic portrait/landscape and A→B→C atomic-publication tests, source-consumer identity retention, recoverable fallback, zero geometry CMM requests, and a Release transition smoke with a bright Matte: four source conversions, zero `MatteWithoutPhotoFrames`, about `313 ms` for the final 15 MP transition and a `591 ms` maximum on the 24 MP transition. Acceptance still requires owner visual review/push followed by normal Windows/Linux/macOS CI and the extended native Little CMS `win-x64`/`linux-x64`/`osx-arm64` product-interop matrix. macOS, Linux/X11/Wayland, Windows Advanced Color/HDR, high-depth output, real multi-monitor hardware transition evidence, and managed navigation prefetch remain separate work.
