...
R8-B-W1-F3 is locally ready for owner verification at unchanged `0.1.0.0008`, as a forward correction on committed F2 baseline `8f8ea8f7f263ec29855530e0a37af593dabe728b`. W1/F1/F2 respectively exposed Black, incomplete-coverage, and blur-to-sharp raster-replacement regressions because monitor pixels were keyed by viewport geometry. F3 replaces that family with one encoded-size managed source keyed only by source and destination; the existing photo renderer remains the sole Fit/100%/zoom/pan/resize/Peek geometry and sampling authority. A 15 MP Release interaction run recorded 261 managed frames, one CMM request/completion, and zero geometry requests. Advanced Color/HDR, invalid/unavailable profiles, unsupported source meaning, and missing native runtime retain the existing presentation recoverably. Color Picker remains reference sRGB and Histogram remains source-domain. R8-B-P1 is accepted at `932c1285394d6bfa6c10cc75c1810c882eef7c6c`; the test correction is accepted at `90d733254790d152cc5d298f39c23e2b8171b249`; R8-B-N1 is accepted at `5155b7806703a657d89ab2923fd2936814a37a16` with green normal and native hosted matrices.
...
R8-B-W1-F3 is the current owner-review focus. Local Windows evidence covers real HWND/profile resolution, Advanced Color disabled classification, app-local runtime loading, production matrix/TRC and LUT destination transforms, alpha, source/destination latest-wins, same-size spatial parity across 50 geometry states and five orientations, actual GPU/runtime-shader feasibility evidence, and a 15 MP Release zoom/pan/Fit/100%/Peek/resize run with one CMM operation and zero geometry requests. Acceptance still requires owner visual review/push followed by normal Windows/Linux/macOS CI and the extended native Little CMS `win-x64`/`linux-x64`/`osx-arm64` product-interop matrix. macOS, Linux/X11/Wayland, Windows Advanced Color/HDR, high-depth output, real multi-monitor hardware transition evidence, and navigation latency on very large sources remain separate work.
