...
The Stage suite includes R5-P1 synchronization evidence that current Ambient preparation/publication occurs before a blocked adjacent-preload barrier, matching cached neighbor Ambient transfers without another prepare call, and obsolete current work cannot publish over the latest selection. These are ordering assertions, not wall-clock thresholds.
