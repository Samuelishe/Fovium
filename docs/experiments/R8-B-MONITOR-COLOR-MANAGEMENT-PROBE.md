...
The owner subsequently accepted R8-B-P1 and R8-B-N1. Accepted N1 commit `5155b7806703a657d89ab2923fd2936814a37a16` owns pinned Little CMS 2.19 under [`../../eng/native/lcms2/`](../../eng/native/lcms2/) and has green hosted `win-x64`, `linux-x64`, and `osx-arm64` native evidence plus normal CI. R8-B-W1 then implemented the Windows ordinary-SDR product path at `0.1.0.0008`. Owner runtime evidence rejected the original viewport raster, its F1 proxy correction, and its F2 base/detail correction because geometry-dependent raster replacement respectively exposed Black, incomplete coverage, and blur-to-sharp/stitch artifacts. R8-B-W1-F3 therefore supersedes only that historical rendering recommendation with one encoded-size source/destination managed image drawn through the canonical viewport renderer; the focused evidence is [`R8-B-W1-SEAMLESS-RENDERING-PROBE.md`](R8-B-W1-SEAMLESS-RENDERING-PROBE.md). This appendix does not rewrite the original P1 findings. macOS/Linux monitor output and Windows Advanced Color/HDR remain unimplemented.
