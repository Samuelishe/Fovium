...
The production locator accepts only the exact Fovium bundle under `runtimes/<rid>/native`, loads dependencies and libheif by absolute path, requires libheif 1.23.1 with HEVC/AV1 decoders and no HEVC/AV1 encoders, and has no system fallback. Tracked project-authored fixtures cover 8-bit RGB HEIF/AVIF, AVIF alpha, rotation, mirror, real 10-bit rejection, PQ/HLG signaling rejection, sequence rejection, and malformed input. The native workflow builds/audits/smokes each mandatory RID, materializes its bundle, and runs the actual production backend tests. R7-C's hosted product evidence is owner-accepted green on `win-x64`, `linux-x64`, and `osx-arm64`.
