...
R5-P1 Windows owner-corpus evidence reproduced the old scheduling defect as an approximately `134 ms` exact-Black edge interval during a deliberately close pair of 24 MP navigation requests. After current-first scheduling, real decoded current-image preparation measured `8.01–11.70 ms` with coordinator publication gaps of `8.08–15.36 ms` across five `3936×2624` derivatives, and `14.34–15.25 ms` with gaps of `14.44–15.38 ms` across five `6000×4000`/`4000×6000` sources. Eight ordinary forward plus four reverse 24 MP transitions and four forward plus two reverse 10 MP transitions produced no exact-Black edge sample. A deliberately rapid double transition still produced an approximately `100 ms` safe Black fallback after outrunning speculative readiness; no stale/mismatched Ambient appeared. The normal path no longer justified a progressive session hook or tiny placeholder. These are single-machine observations, not universal frame-time thresholds.
