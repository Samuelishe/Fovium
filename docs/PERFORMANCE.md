...
R5 presenter rendering adds no decode, file I/O, Ambient preparation, photo-cache budget, or large offscreen compositor. It draws bounded managed primitives in one clipped post-photo pass; documents cap at 2048 elements per image and brush drafts at 8192 points. Highlight movement invalidates only the viewport presentation.
...
R5 Windows Release smoke at `RenderScaling = 1.00` visually exercised four tools, highlight, panel hide/show, Clear, zoom/pan, fullscreen, Peek, image-specific Blink swapping, focus loss, Settings persistence, and restart cleanup. No frame-time benchmark was added; no visible interaction stall was observed, which is qualitative local evidence rather than a threshold.
