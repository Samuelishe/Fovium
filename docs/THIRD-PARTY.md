...
| Little CMS | Marti Maria Saguer / Little CMS project | MIT | Native source-build prerequisite for future Monitor Color Management | [Repository](https://github.com/mm2/Little-CMS), [release](https://github.com/mm2/Little-CMS/releases/tag/lcms2.19) | `2.19`, tag `lcms2.19`, commit `b76633e60c8387a77268fb3359277ca25b5fd75c` | R8-B-P1 used a disposable ignored reference; R8-B-N1 adds the separate reproducible tracked build owner under `eng/native/lcms2/`. The core shared runtime is app-local, while tools/tests/static libraries and optional image-library/GPL plug-ins are excluded. Windows `win-x64` is locally proven; required `linux-x64` and `osx-arm64` remain pending hosted build/run evidence. This is not yet a production Fovium runtime dependency |
