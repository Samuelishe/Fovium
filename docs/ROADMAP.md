...
R8-B-W1 is locally complete at `0.1.0.0008` and ready for owner/hosted verification. It implements enabled-by-default, photograph-only Windows ordinary-SDR management through the active monitor ICC and app-local Little CMS, with bounded viewport presentation, latest-wins publication, Settings opt-out, and explicit fallback. Later macOS, Linux X11, Wayland, Windows Advanced Color/HDR, high-depth output, and broader real multi-monitor validation remain separate stages described by the [probe](experiments/R8-B-MONITOR-COLOR-MANAGEMENT-PROBE.md). Do not begin another platform automatically; general format expansion (BMP, ICO, SVG, bounded PSD composite, and later formats) remains separate.
